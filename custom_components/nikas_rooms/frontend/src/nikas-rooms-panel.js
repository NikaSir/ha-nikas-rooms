...
class NikasRoomsV11 extends HTMLElement {
  ...
  mountShell() {
    ...
    this.shadowRoot.innerHTML = `
      ...
      <div class="nikas-shell app" style="--nikas-shell-tab-count:4">
        ...
        <nav class="nikas-shell__tabs tabs" aria-label="Основные панели NikaS">
          <button class="nikas-shell__tab" type="button" data-path="${this.houseRoute()}" aria-label="Дом"><ha-icon icon="mdi:home-outline"></ha-icon><small>Дом</small></button>
          <button class="nikas-shell__tab" type="button" data-route-kind="overview" aria-label="Помещения"><ha-icon icon="mdi:floor-plan"></ha-icon><small>Помещения</small></button>
          <button class="nikas-shell__tab" type="button" data-path="/dashboard-actions/home" aria-label="Действия"><ha-icon icon="mdi:lightning-bolt-outline"></ha-icon><small>Действия</small></button>
          <button class="nikas-shell__tab" type="button" data-path="/dashboard-infrastructure/overview" aria-label="Инфраструктура"><ha-icon icon="mdi:server-network"></ha-icon><small>Инфра</small></button>
        </nav>
  ...
  renderRoute(force = false, targetRoute = null) {
    ...
    this._canvas.className = `nikas-shell__canvas canvas ${route.kind}`;
  ...
  buildRouteViews() {
    ...
    this._canvas.innerHTML = `<div class="nikas-shell__content">${panels.join("")}</div>`;
